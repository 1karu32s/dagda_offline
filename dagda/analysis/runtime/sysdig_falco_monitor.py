...
class SysdigFalcoMonitor:
    ...
    def pre_check(self):
        if not InternalServer.is_external_falco():
            ...
            # Check Docker driver
            docker_client = self.docker_driver.get_docker_client()
            if docker_client is None:
                raise DagdaError('Error while fetching Docker server API version.')

            # Check if the falcosecurity/falco image exists locally
            try:
                docker_client.inspect_image('falcosecurity/falco:0.29.0')
                DagdaLogger.get_logger().info("Local image falcosecurity/falco:0.29.0 found, skipping pull step.")
            except docker.errors.ImageNotFound:
                # Docker pull for ensuring the falcosecurity/falco image
                DagdaLogger.get_logger().info("Local image not found, pulling falcosecurity/falco:0.29.0.")
