...
class DBComposer:
    ...
    def compose_vuln_db(self):
        ...
        csv_content = get_http_resource_content(
            'https://gitlab.com/exploit-database/exploitdb/-/raw/main/files_exploits.csv?ref_type=heads&inline=false')
        ...
        bz2_file = get_http_resource_content('https://access.redhat.com/security/data/archive/oval_v1_20230706.tar.gz')
        ...
        # # Updating BugTraqs from http://www.securityfocus.com/    --- No longer available
    ...
    def _threaded_preprocessed_bid_gathering(mongoDbDriver, max_bid):
        ...
        # Adding BIDs
        if InternalServer.is_debug_logging_enabled():
            DagdaLogger.get_logger().debug('Downloading 20180328_sf_db.json.gz from github ...')
        with open('20180328_sf_db.json.gz', 'rb') as bugtraqsfile:
            content = bugtraqsfile.read()
        compressed_file = io.BytesIO(content)
        # compressed_file = io.BytesIO(get_http_resource_content(
        #     "https://raw.githubusercontent.com/eliasgranderubio/bidDB_downloader/master/bonus_track/20180328_sf_db.json.gz"))
        # compressed_file = io.BytesIO(get_http_resource_content(
        ...
        # Insert BIDs
        if InternalServer.is_debug_logging_enabled():
            DagdaLogger.get_logger().debug('Adding BugTraqs from 20180328_sf_db.json.gz ...')
        for bid_items_list in bid_items_array:
